...
class Net(nn.Module):
    def __init__(self):
        ...
        self.dropout = nn.Dropout(0.5)  # histburt nuren nalam
...
def train(loader, criterion, optimizer):
    ...
    for inputs, labels in loader:
        ...
        outputs = model(inputs) #vector 30 ,
...
#     torch.multiprocessing.freeze_support()
    ...
    test_loader = GCommandLoader('./ML4_dataset/data/test')
    ...
    outputs = []
    for x in test_loader:
        inp = x[0].to(device).reshape([1, x[0].shape[0], x[0].shape[1], x[0].shape[2]])
        outputs.append(torch.argmax(model(inp), 1).item())
#x[0] photo  and size
